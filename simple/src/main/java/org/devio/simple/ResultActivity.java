...
public class ResultActivity extends Activity {
    ...
    private void showImg() {
        ...
        for (int i = 0, j = images.size(); i < j - 1; i += 2) {
            ...
            Picasso.with(this).load(new File(images.get(i).getCompressPath())).into(imageView1);
            Picasso.with(this).load(new File(images.get(i + 1).getCompressPath())).into(imageView2);
            linearLayout.addView(view);
        }
        if (images.size() % 2 == 1) {
            View view = LayoutInflater.from(this).inflate(R.layout.image_show, null);
            ImageView imageView1 = (ImageView) view.findViewById(R.id.imgShow1);
            Picasso.with(this).load(new File(images.get(images.size() - 1).getCompressPath())).into(imageView1);
            linearLayout.addView(view);
        }
